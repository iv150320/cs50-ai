...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # 4) Mark any additional cells as safe or mines based on knowledge
        self._update_knowledge()
        
        # 5) Infer new sentences using subset method
        self._infer_new_sentences()

    def _update_knowledge(self):
        """
        Update knowledge by marking cells as safe or mines based on
        what can be inferred from existing sentences.
        """
        changed = True
        while changed:
            changed = False
    ...
    def _infer_new_sentences(self):
        ...
        If we have sentence1 where cells1 is a subset of cells2,
        then we can infer: cells2 - cells1 = count2 - count1
        """
        changed = True
        while changed:
            changed = False
            ...
            for sentence1 in self.knowledge:
                for sentence2 in self.knowledge:
                    ...
                    # Check if sentence1.cells is a proper subset of sentence2.cells
                    if sentence1.cells and sentence2.cells:
                        if sentence1.cells < sentence2.cells:
                            # Infer new sentence
                            new_cells = sentence2.cells - sentence1.cells
                            new_count = sentence2.count - sentence1.count
                            
                            # Only add if valid and not already in knowledge
            ...
            # Update knowledge after adding new sentences
            if changed:
                self._update_knowledge()
    ...
    def make_random_move(self):
        ...
        """
        # Get all possible moves (not moved, not a mine)
        possible_moves = []
        for i in range(self.height):
            for j in range(self.width):
                cell = (i, j)
